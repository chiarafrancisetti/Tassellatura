...
function draw() {
	...
	for (let j=0; j<numY; j++) {
		for (let i=0; i<numX; i++) {
			...
			push()
			translate(piastrella.width * i, piastrella.height * j)

			if ((i + j) % 2 == 0) { 
				image(piastrella, 0, 0)
			} else {
				scale(-1, 1)
				image(piastrella, -piastrella.width , 0)
			}
			pop()
		}
	}
	...
	//image(piastrella, piastrella.width * 1, piastrella.height * 0)
